...
void skiplist::insertNode(double score,const string& key) {
    ...
    for(int i = lvl_-1;i >= 0;i--) {
        ...
        while(cur->level_[i]->forward_ != NULL && (cur->level_[i]->forward_->score_ < score || (cur->level_[i]->forward_->score_ == score && cur->level_[i]->forward_->key_ != key))) {
    ...
    if(cur == NULL || cur->score_ != score) {
        ...
        for(int i = 0;i < lvl_;i++) {
            ...
            node->level_[i]->span_ = update[i]->level_[i]->span_ - (rank[0] - rank[i]);
...
bool skiplist::isInRange(rangeSpec& range) {
    ...
    if (cur == NULL || !valueBmin(cur->score_,range))
        return false;
    cur = head_->level_[0]->forward_;
    if (cur == NULL || !valueSmax(cur->score_,range))
        return false;
    return true;
}
//获得满足范围的第一个节点
skiplistNode* skiplist::getNodeFirstRange(rangeSpec& range) {
    ...
    for (int i = lvl_-1; i >= 0; i--) {
        while (cur->level_[i]->forward_ != NULL && !valueBmin(cur->level_[i]->forward_->score_,range))
            cur = cur->level_[i]->forward_;
    }
    cur = cur->level_[0]->forward_;
...
skiplistNode* skiplist::getNodeLastRange(rangeSpec& range) {
    ...
    if (!isInRange(range)) return NULL;
    for (int i = lvl_-1; i >= 0; i--) {
        while (cur->level_[i]->forward_ && valueSmax(cur->level_[i]->forward_->score_,range))
            cur = cur->level_[i]->forward_;
    }
...
int skiplist::getCountRange(rangeSpec& range) {
    ...
    //寻找左边界
    for (int i = lvl_-1; i >= 0; i--) {
        while (cur->level_[i]->forward_ && (range.minex_ ? cur->level_[i]->forward_->score_ <= range.min_ : cur->level_[i]->forward_->score_ < range.min_))
            cur = cur->level_[i]->forward_;
        update[i] = cur;
    }
    cur = cur->level_[0]->forward_;
    //寻找右边界
    while (cur && (range.maxex_ ? cur->score_ < range.max_ : cur->score_ <= range.max_)) {
        count++;
        cur = cur->level_[0]->forward_;
    }
    return count;
...
void skiplist::deleteNodeRange(rangeSpec& range) {
    ...
    while (cur && (range.maxex_ ? cur->score_ < range.max_ : cur->score_ <= range.max_))
    {
        ...
        delete cur;
        while(lvl_ > 1 && head_->level_[lvl_-1]->forward_ == NULL)
            lvl_--;
        length_--;
        cur = tmp;
    }
