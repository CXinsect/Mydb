...
void Accept::sendInLoop(const std::string& message) {
  ...
  if (!channelAccepted_->isWriting() && output_.getReadableBytes() == 0) {
    nwrite =
        ::write(channelAccepted_->getSockfd(), message.data(), message.size());
    if (nwrite >= 0) {
      if ((int)nwrite >= 0) {
        std::cout << "The data has not been sent, waiting for the next time to "
                     "continue"
                  << std::endl;
      }
    } else {
...
void Accept::handleWrite() {
  if (!channelAccepted_->isWriting()) {
    ssize_t n = ::write(channelAccepted_->getSockfd(), output_.peek(),
    ...
    if (n > 0) {
      ...
      std::cout << "[Accept::handwrite]: " << __LINE__ << strerror(errno)
                << std::endl;
    }
  } else {
    std::cout << "[Accept::handwrite]: Connection has been closed " << __LINE__
              << std::endl;
  }
}
void Accept::handleClose() {
  ...
  std::cout << "Get(): " << channelAccepted_.get()->getSockfd() << " : "
            << channelAccepted_->getSockfd() << std::endl;
