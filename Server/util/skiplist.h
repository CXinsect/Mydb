...
class skiplistNode {
    public:
        skiplistNode(double score,const string& key,int level) {
            this->score_ = score;
            this->key_ = key;
            level_ = unique_ptr<unique_ptr<skiplistLevel>[]>(new unique_ptr<skiplistLevel>[level+1]);
...
struct rangeSpec {
    rangeSpec(double min,double max) : min_(min),max_(max),minex_(true),maxex_(true) {}
...
class skiplist {
    public:
        skiplist(int max,float p) : maxlvl_(max),
                                    p_(p),
                                    lvl_(1),
                                    length_(0),
